...
class FewShotAttributionDataset(BaseFewShotDataset):
...
@DATASETS.register_module()
class FewShotAttributionCopyDataset(FewShotAttributionDataset):
    ...
    Args:
        ann_cfg (list[dict] | dict): contain `data_infos` from other
            dataset. Example: [dict(data_infos=FewShotAttributionDataset.data_infos)]
    """
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        Args:
            ann_cfg (list[dict] | dict): contain `data_infos` from other
                dataset. Example:
                [dict(data_infos=FewShotAttributionDataset.data_infos)]
        ...
        if isinstance(ann_cfg, dict):
            assert ann_cfg.get('data_infos', None) is not None, \
                ...
                f'FewShotAttributionCopyDataset require data_infos.'
...
@DATASETS.register_module()
class FewShotAttributionDefaultDataset(FewShotAttributionDataset):
    """Dataset with some pre-defined VOC annotation paths.

    :obj:`FewShotAttributionDefaultDataset` provides pre-defined annotation files
    ...
    different from :obj:`FewShotAttributionDataset`. The `ann_cfg' should contain
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotAttributionDataset, self).ann_cfg_parser(new_ann_cfg)
