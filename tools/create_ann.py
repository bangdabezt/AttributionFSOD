...
def convert_jsonl_to_voc(jsonl_file, output_dir, text_file_path):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    all_name = []
    with open(jsonl_file, 'r') as file, open(text_file_path, 'w') as txt_file:
        for line in file:
            ...
            objects = [{"bbox": instance["bbox"], "category": instance["category"]} for instance in data["detection"]["instances"]]
            ...
            if filename not in all_name:
                txt_file.write(f'{filename.split(".")[0]}\n')
            ...
            query_objects = [{"bbox": data["query_file"]["exemplar"][0], "category": data["query_file"]["category"]}]
            ...
            if query_filename not in all_name:
                txt_file.write(f'{query_filename.split(".")[0]}\n')
                all_name.append(query_filename)

# Paths for input and output
jsonl_file_path = './train_filename.jsonl'
output_directory = './Annotations'
text_file_path = './ImageSets/Main/train.txt'

# Convert JSONL to VOC XML
convert_jsonl_to_voc(jsonl_file_path, output_directory, text_file_path)
